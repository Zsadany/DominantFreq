...
public class BulkApplication extends Application {
	...
	private BulkSettingsPanel createSettingsPane() {
		...
		Button buttonToProcessEmAll = createStylishButton("Process All ECGs", Style.PROCESS_BUTTON_STYLE.toString());
		buttonToProcessEmAll.setOnAction(localHandlers.processAll);
		Button saveButton = createStylishButton("Save Results", Style.SAVE_BUTTON_STYLE.toString());
		saveButton.setOnAction(localHandlers.save);
		Button closeButton = createStylishButton("Exit", Style.CLOSE_BUTTON_STYLE.toString());
